...
xdnd_init(DndClass * dnd, Display * display)
{
  ...
  dnd->text_uri_list = XInternAtom(dnd->display, "text/plain", False);
  dnd->dnd_data = XInternAtom(dnd->display, "dnd_data", False);
  xdnd_reset(dnd);
}
...
// Atom ** typelist must be free'd by user
void
xdnd_get_type_list(DndClass * dnd, Window window, Atom ** typelist)
{
  ...
  *typelist = 0;
  ...
  tl = (Atom *)(new char[(count + 1) * sizeof (Atom)]);
  *typelist = tl;
  a = (Atom *) data;
  for (i = 0; i < count; i++) {
    (*typelist)[i] = a[i];
