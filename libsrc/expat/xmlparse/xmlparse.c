...
static int
defineAttribute(EXPAT_ELEMENT_TYPE *type, ATTRIBUTE_ID *, int isCdata, int isId, const XML_Char *dfltValue);
static enum XML_Error
...
static int setElementTypePrefix(XML_Parser parser, EXPAT_ELEMENT_TYPE *);
static enum XML_Error
...
typedef struct {
  ...
  EXPAT_ELEMENT_TYPE *m_declElementType;
...
static enum XML_Error storeAtts(XML_Parser parser, const ENCODING *enc,
				...
				BINDING **bindingsPtr)
{
  EXPAT_ELEMENT_TYPE *elementType = 0;
  int nDefaultAtts = 0;
  ...
  if (tagNamePtr) {
    elementType = (EXPAT_ELEMENT_TYPE *)lookup(&dtd.elementTypes, tagNamePtr->str, 0);
    if (!elementType) {
      tagNamePtr->str = poolCopyString(&dtd.pool, tagNamePtr->str);
      if (!tagNamePtr->str)
	return XML_ERROR_NO_MEMORY;
      elementType = (EXPAT_ELEMENT_TYPE *)lookup(&dtd.elementTypes, tagNamePtr->str, sizeof(EXPAT_ELEMENT_TYPE));
      if (!elementType)
        return XML_ERROR_NO_MEMORY;
	 ...
	 const char **nextPtr)
{
  ...
  for (;;) {
	...
	*eventPP = next;
	...
	if (!name)
	  return XML_ERROR_NO_MEMORY;
	declElementType = (EXPAT_ELEMENT_TYPE *)lookup(&dtd.elementTypes, name, sizeof(EXPAT_ELEMENT_TYPE));
	if (!declElementType)
	  return XML_ERROR_NO_MEMORY;
...
static int
defineAttribute(EXPAT_ELEMENT_TYPE *type, ATTRIBUTE_ID *attId, int isCdata, int isId, const XML_Char *value)
{
...
static int setElementTypePrefix(XML_Parser parser, EXPAT_ELEMENT_TYPE *elementType)
{
  const XML_Char *name;
...
static void dtdDestroy(DTD *p)
{
  ...
  for (;;) {
    EXPAT_ELEMENT_TYPE *e = (EXPAT_ELEMENT_TYPE *)hashTableIterNext(&iter);
    if (!e)
      break;
...
static int dtdCopy(DTD *newDtd, const DTD *oldDtd)
{
  ...
  for (;;) {
    int i;
    EXPAT_ELEMENT_TYPE *newE;
    const XML_Char *name;
    const EXPAT_ELEMENT_TYPE *oldE = (EXPAT_ELEMENT_TYPE *)hashTableIterNext(&iter);
    if (!oldE)
      break;
    name = poolCopyString(&(newDtd->pool), oldE->name);
    if (!name)
      return 0;
    newE = (EXPAT_ELEMENT_TYPE *)lookup(&(newDtd->elementTypes), name, sizeof(EXPAT_ELEMENT_TYPE));
    if (!newE)
      return 0;
