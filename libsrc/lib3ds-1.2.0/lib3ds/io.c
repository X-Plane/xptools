...
lib3ds_io_read_intb(Lib3dsIo *io)
{
  ...
  ASSERT(io);
  lib3ds_io_read(io, (unsigned char *) &b, 1);
  return(b);
}
...
lib3ds_io_read_string(Lib3dsIo *io, char *s, int buflen)
{
  ...
  ASSERT(io);
  for (;;) {
    if (lib3ds_io_read(io, (unsigned char *) &c, 1)!=1) {
      return LIB3DS_FALSE;
    }
    *s++ = c;
    if (!c) {
...
{
  ASSERT(io);
  if (lib3ds_io_write(io, (unsigned char*) &b, 1)!=1) {
    return(LIB3DS_FALSE);
  }
  return(LIB3DS_TRUE);
}
...
lib3ds_io_write_string(Lib3dsIo *io, const char *s)
{
  ASSERT(s);
  ASSERT(io);
  lib3ds_io_write(io, (unsigned char *) s, strlen(s)+1);
  return(!lib3ds_io_error(io));
}
