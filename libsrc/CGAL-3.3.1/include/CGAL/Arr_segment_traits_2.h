...
public:
  ...
  public:
	...
	if(!(Segment_assertions::_assert_is_point_on (p, cv,
                                                  Has_exact_division()) &&
         compare_xy(cv.left(), p) == SMALLER &&
         compare_xy(cv.right(), p) == LARGER))
	{
		printf("CV left = %lf,%lf\n",
					CGAL::to_double(cv.left().x()),
					CGAL::to_double(cv.left().y()));
		printf("CV right = %lf,%lf\n",
					CGAL::to_double(cv.right().x()),
					CGAL::to_double(cv.right().y()));
		printf("P = %lf,%lf\n",
					CGAL::to_double(p.x()),
					CGAL::to_double(p.y()));
		printf("assert on point? %s\n", Segment_assertions::_assert_is_point_on (p, cv,
                                                  Has_exact_division()) ? "yes" : "no");
		printf("left compare: %d, right compare: %d\n",
         compare_xy(cv.left(), p),
         compare_xy(cv.right(), p));
