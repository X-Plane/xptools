...
//returns the merged face after the deletion
	  ...
	  ) const
  {
    ...
    do {
      if ((*ec).target() != v)
        {
#if DEV        
          printf("VALIDITY ERROR: %dnth Edge from V does not have V as target!\n", n);
          cout << "location: " << v->point().x() << "," << v->point().y() << "\n";
#endif          
          valid = false;
        }
      ++ec;
	  ...
	  ) const
  {
    ...
    {
#if DEV
    	printf("VALIDITY ERROR: Halfedge's target is not next halfedge's twin's target!\n");
#endif    	
      return false;
    }
    return true;
                ...
                Face_const_handle f) const
  {
    ...
    do {
      ...
      {
#if DEV
        printf("VALIDITY ERROR: Halfedge's face is not right.\n");
        printf("Halfedge from %lf,%lf->%lf,%lf\n", 
